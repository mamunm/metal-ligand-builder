#!/usr/bin/env python3
"""
Complete Metal-Ligand Binding Analysis Example: Ni(II)-Iminodiacetic Acid Complex
...
Iminodiacetic acid (IDA) is a dicarboxylic acid chelating ligand
...
def analyze_ni_iminodiacetic_acid_complex():
    """
    Analyze Ni(II)-Iminodiacetic Acid complex formation and binding.
    
    Iminodiacetic acid can coordinate through:
    ...
    config = ComplexConfig(
        # Experiment settings
        experiment_name="ni_iminodiacetic_acid_binding_analysis",
    ...
    # Iminodiacetic acid SMILES
    ida_smiles = "C(=O)(O)CNCC(=O)O"
    
    complex = MetalLigandComplex(
        ligand_name="iminodiacetic_acid",
        ligand_smiles=ida_smiles,
        metal_symbol="Ni",
        metal_charge=2,      # Ni²⁺
        ligand_charge=0,     # IDA neutral form for structure generation
...
def main():
    ...
    try:
        analyze_ni_iminodiacetic_acid_complex()
    except Exception as e:
