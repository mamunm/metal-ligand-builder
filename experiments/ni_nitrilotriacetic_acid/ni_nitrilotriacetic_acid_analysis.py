#!/usr/bin/env python3
"""
Complete Metal-Ligand Binding Analysis Example: Ni(II)-Nitrilotriacetic Acid Complex
...
Nitrilotriacetic acid (NTA) is a tricarboxylic acid chelating ligand
that can coordinate through carboxyl groups and nitrogen atom.
"""
...
def analyze_ni_nitrilotriacetic_acid_complex():
    """
    Analyze Ni(II)-Nitrilotriacetic Acid complex formation and binding.
    
    Nitrilotriacetic acid can coordinate through:
    - Carboxylic acid oxygen atoms
    - Central nitrogen atom
    
    This analysis will generate structures and optimize them to find stable
    coordination geometries around Ni(II).
    """
    ...
    config = ComplexConfig(
        # Experiment settings
        experiment_name="ni_nitrilotriacetic_acid_binding_analysis",
    ...
    # Nitrilotriacetic acid SMILES
    nta_smiles = "C(=O)(O)CN(CC(=O)O)CC(=O)O"
    
    complex = MetalLigandComplex(
        ligand_name="nitrilotriacetic_acid",
        ligand_smiles=nta_smiles,
        metal_symbol="Ni",
        metal_charge=2,      # Ni²⁺
        ligand_charge=0,     # NTA neutral form for structure generation
...
def main():
    ...
    try:
        analyze_ni_nitrilotriacetic_acid_complex()
    except Exception as e:
